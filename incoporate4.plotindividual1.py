...
# --- Plot 2a: Performance of All Models (Enhanced with Log Scale) ---
print("Generating Plot 2a: Model Performance (Enhanced Log Scale View)...")
if not df_all_models.empty:
    fig2a, ax2a = plt.subplots(figsize=(12, 8))
    
    # Create the scatter plot
    sns.scatterplot(x='RMSE', y='Cost', data=df_all_models, hue='Model Type', style='Model Type',
                    s=250, ax=ax2a, palette='viridis')
    
    # Apply a logarithmic scale to both axes
    # This will compress the large values and expand the clustered low values
    ax2a.set_xscale('log')
    ax2a.set_yscale('log')
    
    # Re-format the tick labels to be readable numbers instead of scientific notation
    ax2a.get_xaxis().set_major_formatter(plt.FuncFormatter(lambda x, p: format(int(x), ',')))
    ax2a.get_yaxis().set_major_formatter(plt.FuncFormatter(lambda x, p: f'${format(int(x), ",")}'))

    # Add text annotations to label key points, preventing overlap
    # We can add a small offset to each label
    for i in range(len(df_all_models)):
        # Get the model's name and coordinates
        model_name = df_all_models['Model Type'].iloc[i]
        x_coord = df_all_models['RMSE'].iloc[i]
        y_coord = df_all_models['Cost'].iloc[i]
        
        # Add a simplified label next to the point
        ax2a.text(x_coord * 1.02, y_coord, # Position the text slightly to the right
                  s=model_name, # The text to display
                  fontdict=dict(color='black', size=10))

    ax2a.set_title('Performance of Models Trained with Different Objectives (Log Scale)', fontsize=16)
    ax2a.set_xlabel('Resulting Forecast RMSE (MW) [Log Scale]', fontsize=12)
    ax2a.set_ylabel('Resulting System Cost ($) [Log Scale]', fontsize=12)
    
    # We can hide the default legend because the annotations now label the points
    ax2a.get_legend().remove()
    ax2a.grid(True, which='both', linestyle='--') # Grid for both major and minor ticks
    
    plt.tight_layout()
    plt.savefig('plot2a_model_performance_log_scale.png', dpi=300)
    plt.show()
